...
wp_enqueue_script(
    'jquery214',
    get_bloginfo('template_directory') . '/scripts/jquery-2.1.4.min.js',
    array( 'jquery' ),
    '',
    true
);
wp_enqueue_script(
    'myscript',
    get_bloginfo('template_directory') . '/scripts/script-min.js',
    array( 'jquery' ),
    '',
    true
);
...
wp_localize_script( 'myscript', 'myobject', $translation_array );
}
add_action('wp_enqueue_scripts', 'my_scripts_method');

function my_login_stylesheet() {
    wp_enqueue_style( 'custom-login', get_template_directory_uri() . '/style-login.css' ); 
    wp_enqueue_script( 'custom-login-typekit', 'https://use.typekit.net/wpg1oxz.js' );
    wp_enqueue_script( 'custom-login', get_template_directory_uri() . '/scripts/login-script.js' );
}
...
register_sidebar( array('name' => 'Home',
  'id' => 'home',
  'description'  => __( 'Contenido del Slider.' ),
...
register_sidebar( array('name' => 'Kamra',
  'id' => 'Kamra',
  'description'  => __( 'Contenido del Slider.' ),
  'before_widget' => '<div class="kamra-widget">',
  'after_widget' => '</div>',
  'before_title' => '<span class="hidden">',
  'after_title' => '</span>'
) );
	...
	if ( function_exists( 'register_nav_menus' ) ) {
        ...
        'kamra-menu' => 'KAMRA Menu',
        'refractiva-menu' => 'Cirugia Refractiva Menu'
	  		)
	  	);
	}
	...
	add_filter( 'login_errors', function( $error ) {
     ...
     if ( in_array( 'invalid_username', $err_codes ) ) {
      $error = '<strong>ERROR</strong>: El nombre de usuario o contraseña son invalidos.';
     }
...
add_image_size( 'custom', 900, 380, array( 'center', 'center' ) );
?>